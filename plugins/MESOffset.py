...
class MESOffset(mosPlugin.MESPlugin):
    ...
    def check_mes_star(self, *args):
        ...
        self.logger.info('MESOffset check_mes_star calling MESInterface.check with star_locations %s' % self.star_locations)
    ...
    def check_mes_hole(self, *args):
        ...
        self.logger.info('MESOffset check_mes_hole calling MESInterface.check with hole_locations %s' % self.hole_locations)
    ...
    def end_mesoffset1(self, *args):
        ...
        self.logger.info('MESOffset1 offsets are dx %s pix dy %s pix rotate %s deg' % self.mes_analyze.offset)
        self.database['starhole_chip1'] = self.star_chip1 + 6
    ...
    def end_mesoffset2(self, *args):
        ...
        self.mes_interface.write_to_logfile(self.rootname_logfile,
                                            "MES Offset 2",
                                            self.mes_analyze.offset)
        self.logger.info('MESOffset2 offsets are dx %s pix dy %s pix rotate %s deg' % self.mes_analyze.offset)
    ...
    def check_mes_hole_again(self, *args):
        """ Review the results from mes_hole and offer a chance to retry """
        self.hole_locations = self.mes_locate.output_data
        self.logger.info('MESOffset check_mes_hole_again calling MESInterface.check with hole_locations %s' % self.hole_locations)
        self.mes_interface.check(self.hole_locations,
    ...
    def end_mesoffset3(self, *args):
        ...
        self.mes_interface.write_to_logfile(self.rootname_logfile,
                                            "MES Offset 3",
                                            self.mes_analyze.offset)
        self.logger.info('MESOffset3 offsets are dx %s pix dy %s pix rotate %s deg' % self.mes_analyze.offset)
        self.database['starhole_chip1'] = self.starhole_chip1 + 2
        self.mes_interface.go_to_mesoffset(2)
    ...
    def process_fits(self, mode, recalc=True, next_step=None):
        ...
        # otherwise, start the appropriate process in a new thread
            ...
            task = lambda: fitsUtils.auto_process_fits(mode,n1,n2,c,i,w,f,e,l,
                                                       next_step=next_step)
