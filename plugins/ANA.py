...
class ANA(GingaPlugin.GlobalPlugin):
    ...
    def mesoffset(self, tag, future, instrument_name=None, star_chip1=None,
        ...
        self.logger.info('ANA MESOffset called')
        ...
        rsinfo = chinfo.opmon.getPluginInfo('MESOffset')
        rsobj = rsinfo.obj

        rsobj.set_params(star_chip1, rootname, c_file, img_dir, exec_mode,
                         mcsred_dir, training_dir, work_dir, wait_gui)

        # Deactivate plugin if one is already running
        pluginName = 'MESOffset'
        if chinfo.opmon.is_active(pluginName):
            self.logger.info('ANA MESOffset calling deactivate')
            chinfo.opmon.deactivate(pluginName)
            self.fv.update_pending()
        ...
        self.logger.info('ANA MESOffset completed')
#END
