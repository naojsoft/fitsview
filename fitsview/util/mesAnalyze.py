...
class MESAnalyze(object):
    ...
    def start(self, star_pos, hole_pos, next_step=None):
        ...
        @param star_pos:
            A 2-3 column (x,y[,r]) array storing the star locations and sizes
        @param hole_pos:
            A 2-3 column (x,y[,r]) array storing the hole locations and sizes
        @param rootname:
    ...
    def set_active_cb(self, _, __, x, y, val):
        """
        Respond to a right or left click on the main ImageViewer by
        altering the datum nearest the cursor
        @param x:
        ...
        """
        distance_from_click = np.hypot(
            self.data[:, 0] - x, self.data[:, 1] - y)
        idx = np.argmin(distance_from_click)
        self.active[idx] = val
        self.logger.info(
            'MESAnalyze data point with index %s set to status %s' %
            (idx, val))
        self.update_plots()
    ...
    def toggle_active_cb(self, event, plt):
        """
        Respond to right or left click on one of the plots by altering
        the datum nearest the cursor
        @param event:
        ...
        self.logger.info(
            'MESAnalyze plot data point with index %s set to status %s' %
            (idx, self.active[idx]))
        self.update_plots()
    ...
    def update_plots(self):
        ...
        try:
            theta = np.mean([math.acos(rot_mat[0, 0]),
                             math.asin(rot_mat[1, 0])])
        except ValueError:
            theta = 0
    ...
    def display_values(self):
        ...
        dx = -yshift + xcenter * \
            math.sin(thetaR) + ycenter * (1 - math.cos(thetaR))
        dy = xshift + xcenter * (math.cos(thetaR) - 1) + \
            ycenter * math.sin(thetaR)
    ...
    def delete_outliers(self):
        """
        Remove any data points with residuals of absolute values greater
        than 1. Also updates the plots
        """
        active = self.active
    ...
    def make_gui_vals(self, orientation='vertical'):
        ...
        txt.set_font(self.manager.NORMAL_FONT)
        txt.set_text(
            "Enter the numbers you see below into the ANA window. dx " +
