...
class MESOffset(mosPlugin.MESPlugin):
    ...
    PARAMS_0 = [
        ...
        {'name': 'c_file',
         'label': "Config File", 'type': str,
         'default': "$DATABASE/ana_apr16.cfg",
    ...
    def __init__(self, fv, fitsimage):
        ...
        # the variables shared between the departments
        self.database = {}
        # the indices of the guis in the stack
        self.stack_idx = {}
    ...
    def set_params(self, star_chip1, rootname, c_file, img_dir, exec_mode,
        ...
        try:
            exec_option = self.PARAMS_0[4]['options'].index(
                exec_mode.capitalize())
        except ValueError:
            exec_option = 0
        ...
        self.logger.info(
            'MESOffset wait_gui input value %s self.wait_gui %s' %
            (wait_gui, self.wait_gui))
        self.logger.info('MESOffset after PARAMS_0 %s' % self.PARAMS_0)

    def stack_guis(self, stack, orientation='vertical'):
        """
        Get the GUIs from all the different departments of mesoffset,
        and stacks them together neatly in one gw.Widget
        """
        stack.remove_all()
    ...
    def begin_mesoffset1(self):
        ...
        """
        # modify the database if necessary, and then absorb all values from the
        # database
        ...
        self.logger.info(
            'Setting MESOffset1 debug mode %s' %
            self.database['debug_mode'])
    ...
    def mes_star(self, *args):
        ...
        self.sbr_data = self.mes_locate.read_sbr_file(os.path.join(
            self.training_dir, self.rootname + ".sbr"), self.logger)
    ...
    def check_mes_star(self, *args):
        ...
        self.logger.info(
            'MESOffset check_mes_star calling MESInterface.check \
             with star_locations %s' % self.star_locations)
    ...
    def process_mask_fits(self, *args):
        ...
        self.logger.info('MESOffset1p5 database %s' % self.database)
        self.__dict__.update(self.database)
    ...
    def check_mes_hole(self, *args):
        ...
        self.logger.info(
            'MESOffset check_mes_hole calling MESInterface.check \
            with hole_locations %s' % self.hole_locations)
    ...
    def end_mesoffset1(self, *args):
        ...
        self.logger.info(
            'MESOffset1 offsets are dx %s pix dy %s pix rotate %s deg' %
            self.mes_analyze.offset)
        self.database['starhole_chip1'] = self.star_chip1 + 6
    ...
    def process_starhole_fits(self, *args):
        """
        Use fitsUtils to combine raw data into a
        compound starhole image
        """
    ...
    def mes_starhole(self, *args):
        ...
        self.logger.info(
            'MESOffset mes_starhole calling self.mes_locate.start with \
            self.hole_locations as %s' % self.hole_locations)
    ...
    def check_mes_starhole(self, *args):
        ...
        self.star_locations = self.mes_locate.output_data[:, :2]
        self.logger.info(
            'MESOffset check_mes_starhole calling MESInterface.check with \
            star_locations %s' % self.star_locations)
        self.mes_interface.check(self.star_locations,
    ...
    def check_mes_hole_again(self, *args):
        ...
        self.hole_locations = self.mes_locate.output_data
        self.logger.info(
            'MESOffset check_mes_hole_again calling MESInterface.check \
            with hole_locations %s' % self.hole_locations)
        self.mes_interface.check(self.hole_locations,
    ...
    def check_mes_starhole_again(self, *args):
        """
        Review the results from mes_starhole and offer a
        chance to retry.
        """
        self.star_locations = self.mes_locate.output_data  # [:,:2]
        self.logger.info(
            'MESOffset check_mes_starhole_again calling MESInterface.check \
            with star_locations %s' % self.star_locations)
        self.mes_interface.check(self.star_locations,
    ...
    def end_mesoffset3(self, *args):
        """
        Finish off the third, fine star-hole location with
        updated masks
        """
        ...
        self.mes_interface.write_to_logfile(self.rootname_logfile,
                                            ...
                                            self.mes_analyze.offset)
        self.logger.info(
            'MESOffset3 offsets are dx %s pix dy %s pix rotate %s deg' %
            self.mes_analyze.offset)
        self.database['starhole_chip1'] = self.starhole_chip1 + 2
        self.mes_interface.go_to_mesoffset(2)
    ...
    def process_fits(self, mode, recalc=True, next_step=None):
        ...
        out_filename = os.path.join(
            self.work_dir, self.rootname + "_" + mode + ".fits")
        ...
        # otherwise, start the appropriate process in a new thread
            ...
            def task(): return fitsUtils.auto_process_fits(
                mode, n1, n2, c, i, w, f, e, log, next_step=next_step)
    ...
    def image_set_cb(self, *args):
        """
        Respond to an image being loaded by executing whatever
        function
        """
        # TO-DO: I don't know why this callback sometimes
        if self.fitsimage.get_image() is None:
            # executes for no reason, so I just ignore it.
            return
