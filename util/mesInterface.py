...
class MESInterface(object):
    ...
    def __init__(self, manager):
        ...
        # intermediate functions to call after waiting
        self.resume_mesoffset = {}
    ...
    def update_parameters(self, getters, write_to_file=False):
        ...
        self.logger.info(
            'MESInterface updating database with values %s' %
            new_params)
    ...
    def return_to_menu_cb(self, *args):
        """
        Go back to the last parameter menu you were at - either 'epar' or
        'wait'
        """
    ...
    def resume_process_cb(self, *args):
        """
        Take the parameters from the waiting gui and resume the current
        process
        """
        try:
    ...
    def check(self, data, last_step=None, next_step=None):
        ...
        @param data:
            The location data in the form of a 2-3 column numpy array
            (x,y[,r])
        @param last_step:
    ...
    def terminate_process_cb(self, *args):
        ...
        wait for the process to receive the signal, and then go to
        the main menu
        """
    ...
    def make_gui_epar(self, idx, orientation='vertical'):
        """
        Construct a GUI for the parameter menu, which prepares to
        launch process
        @param idx:
            The index of this MESOffset process, or None for an
            ambiguous epar
        @param orientation:
            Either 'vertical' or 'horizontal', the orientation of this new GUI
        @returns:
    ...
    def make_gui_look(self, orientation='vertical'):
        ...
        txt.set_font(self.manager.NORMAL_FONT)
        txt.set_text(
            "Look at the results below. If they seem correct, click " +
...
def build_control_layout(controls, callback=None):
    ...
    for i, param in enumerate(controls):
        ...
        if 'options' in param:
            ...
            raise TypeError(
                "{} is not a valid par-type.".format(param['type']))
    ...
    while "$" in filename:
        ...
        # otherwise, raise an error
            ...
            err_msg = (
                "$" +
                var_name +
                " is not a defined variable. Defined " +
                "variables are:\n")
