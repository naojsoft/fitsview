...
class MESLocate(object):
    ...
    def start(self, initial_data, mode, interact2=True,
        ...
        @param initial_data:
            The numpy array containing the approximate positions of the
            relevant objects
        @param mode:
        ...
        self.drag_start = None     # the place for beginning to drag
        # the new obj_arr based on user input and calculations
        self.obj_centroids = np.zeros(self.obj_arr.shape)
        self.square_size = {'star': 30, 'mask': 60, 'starhole': 20}[
            mode]  # the apothem of the search regions
        self.exp_obj_size = {'star': 4, 'mask': 20, 'starhole': 4}[
            mode]  # the maximum expected radius of the objects
        ...
        if mode == 'star':
            ...
            # the maximum expected radius of objects
            self.exp_obj_size = 4
    ...
    def click1_cb(self, _, __, x, y):
        ...
        # if there are things saved ahead of this index (because of undo),
        # clear them
    ...
    def step2_cb(self, *args):
        ...
        self.logger.info(
                'MESLocate mode %s coordinates are %s' %
                (self.mode, str(self.click_history[self.click_index]))
                )
        ...
        # in the event that there are masks here
        #  (because of the Back button)...
        for i in range(self.obj_num):
            for j in range(self.drag_index[i] + 1):
                self.draw_mask(*self.drag_history[i][j], obj_idx=i,
                               drag_idx=j)
    ...
    def next_obj_cb(self, *args):
        ...
        if math.isnan(x1) or math.isnan(
                y1) or math.isnan(x2) or math.isnan(y2):
    ...
    def start_drag_cb(self, c, k, x, y, kind):
        """
        Respond to the mouse finishing a left-drag by finalizing crop
        selection
        @param kind:
            Either 'mask' or 'crop', depending on the mouse button and the
            current selection mode
        @returns:
            True, in order to prevent the other panset callbacks from
            going off
        """
    ...
    def end_drag_cb(self, c, k, xf, yf, kind):
        ...
        # finish the drawing, but make sure nothing is drawn; it won't be
        # visible anyway
        ...
        if self.drag_index[co] < len(self.drag_history[co]):
            self.drag_history[co] = \
                                self.drag_history[co][:self.drag_index[co]]
    ...
    def select_point(self, point, draw_circle_masks=False):
        ...
        color = BOX_COLORS[self.color_index %
                           len(BOX_COLORS)]   # cycles through all the colors
        shapes = []
        for i, viewer in enumerate(self.thumbnails):
            ...
            cropped_data = src_image.cutout_adjust(
                *[int(z) for z in xylimits])[0]
            viewer.set_data(cropped_data)
            self.fitsimage.copy_attributes(
                        viewer,
    ...
    def draw_mask(self, xd1, yd1, xd2, yd2, kind,
                  obj_idx=None, drag_idx=None):
        """
        Draw the giant rectangles around an object being cropped
        @param xd1, yd1, xd2, yd2:
            floats representing the coordinates of the upper-left-hand
            corner (1) and the bottom-right-hand corner (2) of the drag
        @param kind:
            A string - either 'mask' for an ommission or 'crop' for
            an inclusion
        ...
        if kind == 'crop':
            ...
            self.canvas.add(self.dc.Rectangle(
                        xd1,
                        yd1,
                        xd2,
                        yd2,
    ...
    def zoom_in_on_current_obj(self):
        ...
        only see the object at index self.current_obj. Also locates and
        marks it
        """
        x1, y1, x2, y2, r = self.get_current_box()
    ...
    def mark_current_obj(self, obj=None):
        ...
        # if any of the coordinates are NaN, then a red x will be drawn in the
        # middle
    ...
    def make_gui_find(self, orientation='vertical'):
        ...
        @param orientation:
            Either 'vertical' or 'horizontal', the orientation of this new GUI
        @returns:
            A Widgets.Box object containing all necessary buttons, labels,
            etc.
        """
        ...
        txt.set_text("Left click on the object closest to the box " +
                     "labeled '1'. The other objects should appear " +
                     "in the boxes below. Click again to select another " +
                     "position. Click 'Next' below or right-click when " +
                     "you are satisfied with your location.\nRemember " +
                     "- bright areas are shown in white.")
        exp.set_widget(txt)
    ...
    def make_gui_cent(self, orientation='vertical'):
        ...
        txt.set_text("Help the computer find the centroid of this object. " +
                     ...
                     "left-click will crop to selection and middle-click" +
                     " will mask selection, or you can specify a selection" +
                     " option below. Click 'Next' below or right-click " +
                     "when the centroid has been found.")
        exp.set_widget(txt)
    ...
    def read_sbr_file(filename, logger):
        ...
        @returns:
            A numpy array of two columns, containing the first two data
            in each row of the sbr file
        """
        # open the file
        try:
            sbr = open(filename, 'r')
...
def parse_data(data):
    ...
    @returns:
        A numpy array of three columns of floats representing relative
...
def create_viewer_list(n, logger=None, width=120, height=120):
    """
    147x147 is approximately the size it will set it to, but I have to set
    it manually because otherwise it will either be too small or scale to
    the wrong size at certain points in the program. Why does it do this?
    Why can't it seem to figure out how big the window actually is when it
    zooms? I don't have a clue! It just randomly decides sometime after my
    plugin's last init method and before its first callback method, hey,
    guess what, the window is 194x111 now - should I zoom_fit again to match
    the new size? Nah, that would be TOO EASY. And of course I don't even
    know where or when or why the widget size is changing because it DOESN'T
    EVEN HAPPEN IN GINGA! It happens in PyQt4 or PyQt 5 or, who knows, maybe
    even Pyside. Obviously. OBVIOUSLY. GAGFAGLAHOIFHAOWHOUHOUH~~!!!!!
    """
    """
...
def locate_obj(bounds, masks, image, viewer=None,
    ...
    @param bounds:
        ...
        this box
    @param masks:
        A list of tuples of the form (x1, y1, x2, y2, kind) where kind is
        either 'mask' or 'crop' and everything else is floats. Each tuple
        in masks is one drag of the mouse that ommitted either its interior
        or its exterior
    @param image:
    ...
    while search_radius >= min_search_radius or has_not_executed_yet:
        ...
        while np.hypot(x_cen - old_x_cen, y_cen - old_y_cen) >= 0.5:
            ...
            circle_mask = np.hypot(
                x_arr - x_cen, y_arr - y_cen) > search_radius
            ...
            # now try a center-of-mass calculation to find the size and
            # centroid
            try:
                old_x_cen = x_cen
                old_y_cen = y_cen
...
def empty_circle(x, y, r, a, dc):
    ...
    vertices = [(x + a, y + a), (x + a, y - a), (x - a, y - a),
                (x - a, y + a), (x + a, y + a)]
    # draw the circle
