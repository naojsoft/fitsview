...
def auto_process_fits(mode, n1, n2, c, i, w, f, t, log=nothing, next_step=None):
    """
...
def make_mosaic(im_type, frnum, input_data, c_file, work_dir, terminate, log=nothing):
    ...
    mosaic_data[0] = transform(basename1, 1, input_data[0], config[2], config[3])
    if terminate.is_set():  return
    mosaic_data[1] = transform(basename2, 1, input_data[1], config[4], config[5])
    if terminate.is_set():  return
    
    log("Correcting for more distortion...")
    mosaic_data[0] = transform(basename1, 2, mosaic_data[0], config[8], config[9])
    if terminate.is_set():  return
    mosaic_data[1] = transform(basename2, 2, mosaic_data[1], config[10], config[11])
    if terminate.is_set():  return
    
    # combine and rotate the images
    log("Combining the chips and applying bad pixel mask...")

    combined_image = combine_and_apply_mask(basename1, basename2, mosaic_data[0], mosaic_data[1],
                                            config[12], config[13])
...
def combine_and_apply_mask(basename1, basename2, input1, input2, filename_mask1, filename_mask2, mask_val=0):
    """
